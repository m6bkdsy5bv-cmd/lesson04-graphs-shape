...
# ------------------------------------------------------------
# 8. 스크린수가 많은 영화는 상영횟수도 많은가 - 산점도
# ------------------------------------------------------------
st.header("8️⃣ 스크린수가 많은 영화는 상영횟수도 많은가")

fig_scatter_scrn_show = px.scatter(
    df,
    x="first_scrn",
    y="first_show",
    hover_name="movieNm",
    title="스크린수가 많은 영화는 상영횟수도 많은가",
    labels={"first_scrn": "개봉일 스크린수", "first_show": "개봉일 상영횟수"},
)
fig_scatter_scrn_show.update_traces(
    hovertemplate="영화명: %{hovertext}<br>스크린수: %{x}<br>상영횟수: %{y}<extra></extra>"
)

st.plotly_chart(fig_scatter_scrn_show, use_container_width=True)
insight_box("insight_8_scrn_show_scatter")
